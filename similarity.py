...
def similarity_cal_udf(origin, standard):
	...
	return 0.01 * dg + 0.1 * sp + 0.6 * pq + 0.1 * mn + 0.1 * pd + 0.1 * max(mfc, mfe)


def similarity(spark, df_cleanning, df_standard):
	 df_result = df_cleanning.join(df_standard, df_cleanning.MOLE_NAME == df_standard.MOLE_NAME_STANDARD, how="left").orderBy("PACK_ID_CHECK").na.fill("") \
	 ...
	 df_result = df_result.withColumn("SIMILARITY", similarity_cal_udf(df_result.ORIGIN, df_result.STANDARD))
	 return df_result